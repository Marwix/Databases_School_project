...
CREATE VIEW PathToGraduation AS
    ...
    SELECT IDNR AS student,
        ...
        CASE WHEN BOOL(totalCredits >= 100 AND COALESCE(mandatoryLeft,0) = 0 AND COALESCE(mathCredits,0) >= 20 AND COALESCE(researchCredits,0) >= 10 AND count(seminarcourses) >= 1) THEN TRUE ELSE FALSE END AS qualified
        FROM students
